		...
		while ( $loop->have_posts()) : $loop->the_post(); $i++;
			...
			<article class="'.$classes.'">
				...
				if ( has_post_thumbnail() ) {
					$catchresponsive_page_slider .= '<a title="' . $title_attribute . '" href="' . esc_url( get_permalink() ) . '">
						'. get_the_post_thumbnail( $post->ID, 'catchresponsive-slider', array( 'title' => $title_attribute, 'alt' => $title_attribute, 'class'	=> 'attached-page-image' ) ).'
					</a>';
				}
				else {
					//Default value if there is no first image
					$catchresponsive_image = '<img class="pngfix wp-post-image" src="'.esc_url( get_template_directory_uri() ).'/images/gallery/no-featured-image-1200x514.jpg" >';

					//Get the first image in page, returns false if there is no image
					$catchresponsive_first_image = catchresponsive_get_first_image( $post->ID, 'catchresponsive-slider', array( 'title' => $title_attribute, 'alt' => $title_attribute, 'class' => 'attached-page-image' ) );
				...
				<div class="entry-container">
					<header class="entry-header">
						...
						<div class="screen-reader-text">'.catchresponsive_page_post_meta().'</div>
					</header>';
					if ( $excerpt !='') {
