...
if (defined('ABSPATH') && !class_exists('kt_Central_Palette')) {
    define('KT_CENTRAL_PALETTE', '1.12.6');
    ...
    class kt_Central_Palette {
        ...
        public function iris_integration() {
            ...
            $colors = array_pad($colors, 8, '"transparent"');
            print '
<style type="text/css">
.wp-picker-active .iris-picker {
  padding-bottom: ' . (ceil(count($colors) / 8) * 23). 'px !important }
.wp-picker-active .iris-picker  .iris-palette {
  width: 20px !important;
  height: 20px !important;
  margin: 3px 0 0 3px !important }
.wp-picker-active .iris-picker .iris-palette:first-child,
.wp-picker-active .iris-picker .iris-palette:nth-child(9n) {
...
.mce-colorbutton-grid tr:nth-child($rows) td {padding-bottom: 4px}
        ...
        protected function add_palette(&$map, &$palette, $row) {
            ...
            for ($col = 0; $col < $cols; $col++) {
                ...
                list($color, $_, $name) = array_map('esc_js', $color);
                $map[] = '"' . $color . '","' . $name . '"';
            }
        }
...
<p class='devider'><button type=submit' id='kt_action_export' class='button' name='kt_action' value='export-backup' tabindex='9'>" . esc_html__('Download Backup', 'kt-tinymce-color-grid') . '</button></p>';
...
<p><button type='submit' class='button' name='kt_action' value='import-backup' tabindex='10'>" . esc_html__('Upload Backup', 'kt-tinymce-color-grid') . "</button>";
            }
            print '
</p>';
        }
